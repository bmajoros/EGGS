...
bool SignalSensor::consensusOccursAt(const BOOM::String &str,int index)
{
  return 
    consensusLength==0 || 
    consensuses.isDefined(str.c_str(),index,consensusLength);
}
...
{
  SignalPtr dummy=
    new Signal(sequenceLength,finalTransScore,*this,gc);
  return dummy;
}
...
void SignalSensor::sample(Sequence &appendToThis,bool enforceConsensus)
{
  Alphabet &alphabet=DnaAlphabet::global();
  sample(appendToThis);
  int n=consensuses.size();
  if(enforceConsensus && n>0)
    {
      int r=RandomNumber(n);
      StringMap<char>::iterator cur=consensuses.begin(),
	end=consensuses.end();
      for(int i=0; i<r ; ++cur, ++i);
      char *cp=(*cur).first;
      int index=
	appendToThis.getLength()-contextWindowLength+consensusOffset;
      for(int i=0 ; i<consensusLength ; ++i)
	{
	  char c=cp[i];
	  Symbol s=alphabet.lookup(c);
	  appendToThis[index+i]=s;
	}
    }
}
