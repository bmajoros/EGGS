...
void WMM::load(istream &is)
{
  ...
  // Set some parameters
  wheels.resize(contextWindowLength);
  setSignalType(signalType);
  setStrand(strand);
  setSizes(consensusLength,consensusOffset,contextWindowLength);
  setCutoff(cutoff);

  // Load the matrix && set up roulette wheels for sampling
  ...
  for(int x=0 ; x<maxX ; ++x)
	...
	{
	  is >> p;
	  double logProb=p.asDouble();
	  matrix[x][y]=logProb;
	  double prob=exp(logProb);
	  wheels[x].addSector(prob);
	}  
      wheels[x].doneAddingSectors();
    }
}
...
void WMM::sample(Sequence &seq)
{
  int L=getContextWindowLength();
  for(int i=0 ; i<L ; ++i)
    {
      RouletteWheel &wheel=wheels[i];
      Symbol s=wheel.spin();
      seq.append(s);
    }
}
