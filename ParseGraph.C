...
void ParseGraph::toGFF(ostream &os) const
{
  ...
  os<<"##source-version genezilla 1.0"<<endl;
  os<<"##date "<<getDateAndTime()<<endl;
  os<<"##Type ORFgraph"<<endl;
  os<<"# stats: "<<vertices->size()<<" vertices, "<<countEdges()<<" edges"
    <<endl;
  ...
  for(; cur!=end ; ++cur)
    {
      ...
      BOOM::String source;
      if(signal->isLeftTerminus()) source="left_terminus";
      else if(signal->isRightTerminus()) source="right_terminus";
      else source="vertex";
      os<<substrate<<"\t"
	<<source<<"\t"
	...
	<<endl;
      ...
      // Output the edges leaving this vertex
...
ostream &operator<<(const ParseGraph &G,ostream &os)
{
  G.toGFF(os);
  return os;
}
