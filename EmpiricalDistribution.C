...
int EmpiricalDistribution::sample()
{
//    for(int i=0 ; i<v.size() ; ++i)
    //      cout<<"ELEPHANT: "<<v[i]->first<<"="<<v[i]->second<<endl;
    
  // First, choose a bin of the histogram according to the bin probabilities
  ...
  // Now choose an element in the bin using a uniform distribution
  float halfBinSize=binSize/2;
  int begin=v[i]->first-halfBinSize;
  //cout<<"WALRUS: "<<v[i]->first<<" : "<<binSize<<endl;
  int len=BOOM::RandomNumber(binSize)+begin;
  return len;
}
