					...
					const BOOM::String &substrateId)
{
  ...
  return mainAlgorithm(substrate,substrateString);
}
				...
				SignalPtr signal)
{
  ...
  for(; cur!=end ; ++cur)
	...
	isochore->contentToDistribution[contentType]->
	  getLogP(length+pred->getConsensusLength());
      ...
      for(int oldPhase=0 ; oldPhase<3 ; ++oldPhase)
	{
	  ...
	  //###
	  //predScore=(predProp[oldPhase]-pred->priorInductiveScore(oldPhase)+lengthScore+transScore)*(length+pred->getConsensusLength())+pred->priorInductiveScore(oldPhase);
	  //###
				       ...
				       SignalPtr signal)
{
  ...
  for(; cur!=end ; ++cur)
      ...
      for(int phase=0 ; phase<3 ; ++phase)
	{
	  ...
	  //###
	  //predScore=(predProp[phase]-pred->priorInductiveScore(phase)+lengthScore+transScore)*(length+pred->getConsensusLength())+pred->priorInductiveScore(phase);
	  //###
			   ...
			   int phase,ContentType &contentType)
{
  ...
  double lengthPenalty=
    isochore->contentToDistribution[contentType]->getLogP(length);
  double numerator=
			   ...
			   double parseScore)
{
  ...
  SignalPtr firstSignal=thisSignal;
  ...
  while(!path->isEmpty())
	...
	case NEG_PROM:
	  if(thisSignal!=firstSignal)
	    cout << substrateId << '\t' << PROGRAM_NAME << "\tpromoter\t"
		 << thisSignal->getContextWindowPosition()+1 << '\t' 
		 << min(thisSignal->getContextWindowPosition()
	  ...
	  switch(nextType)
	    ...
	    case TAG:
	      cout << substrateId << '\t' << PROGRAM_NAME<<"\tfinal-exon\t"
		   << thisPos+3 << '\t' 
		   << min(nextPos+3,seqLen) << "\t" << exonScore << "\t+\t"
		   << phase << "\t"
		   << "transgrp=" << transcriptId << ";" << endl;
	      break;
	    }
  ...
  cout<<"#   <http://www.sanger.ac.uk/Software/formats/GFF/GFF_Spec.shtml>" 
      << endl;
}
			    ...
			    BOOM::String &seqStr)
{
  ...
  os << "##source-version " << PROGRAM_NAME << " " << VERSION << endl;
  os << "##date " << getDateAndTime();
  os << "##Type DNA" << endl;
  ...
  while(!path->isEmpty())
	...
	{
	case GT:
	  removeStraddlingStops(seqStr,phase,thisPos,nextPos,
				FORWARD_STRAND);
	  break;
	case NEG_AG:
	  removeStraddlingStops(seqStr,phase,thisPos,nextPos,
	...
	case NEG_POLYA:
	  if(thisSignal!=firstSignal)
	    os << substrateId<<'\t'<<PROGRAM_NAME<<"\tpoly-A-signal\t"
	       << thisPos+1 << '\t' 
	       << min(thisPos+1+thisSignal->getConsensusLength(),seqLen)
	       << "\t.\t"<<strand<<"\t.\t"
	       << "transgrp=" 
	...
	case NEG_PROM:
	  if(thisSignal!=firstSignal)
	    os << substrateId << '\t' << PROGRAM_NAME << "\tpromoter\t"
	       << thisSignal->getContextWindowPosition()+1 << '\t' 
	       << min(thisSignal->getContextWindowPosition()
		      +thisSignal->getContextWindowLength(),seqLen)
	       << "\t.\t"<<strand<<"\t.\t"
	...
	case ATG: // initial-exon or single-exon
	  ...
	  switch(nextType)
	    {
	    case GT:
	      os << substrateId<<'\t'<<PROGRAM_NAME<<"\tinitial-exon\t"
		 << thisPos+1 << '\t' 
		 << min(nextPos,seqLen) << "\t" << exonScore << "\t+\t0\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos,min(nextPos,seqLen),0,
			       FORWARD_STRAND);
	      break;
	    case TAG:
	      os << substrateId<<'\t'<< PROGRAM_NAME << "\tsingle-exon\t"
		 << thisPos+1 << '\t' 
		 << min(nextPos+3,seqLen)<<"\t"<<exonScore<<"\t+\t0\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos,min(nextPos,seqLen),
			       0,FORWARD_STRAND);
	      break;
	    }
	  break;
	case AG: // internal-exon or final-exon
	  switch(nextType)
	    {
	    case GT:
	      os << substrateId << '\t' << PROGRAM_NAME 
		 << "\tinternal-exon\t"
		 << thisPos+3 << '\t' 
		 << min(nextPos,seqLen) << "\t" << exonScore << "\t+\t"
		 << phase << "\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos+2,min(nextPos,seqLen),phase,
			       FORWARD_STRAND);
	      break;
	    case TAG:
	      os << substrateId << '\t' << PROGRAM_NAME<<"\tfinal-exon\t"
		 << thisPos+3 << '\t' 
		 << min(nextPos+3,seqLen) << "\t" << exonScore << "\t+\t"
		 << phase << "\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos+2,min(nextPos,seqLen),
			       phase,FORWARD_STRAND);
	      break;
	    }
	  break;
	case NEG_TAG: // final-exon or single-exon
	  ...
	  switch(nextType)
	    ...
	    case NEG_AG:
		 ...
		 << ((getPhase(nextSignal)+1)%3) << "\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos+3,min(nextPos,seqLen),2,
			       REVERSE_STRAND);
	      break;
	    case NEG_ATG:
	      os << substrateId << '\t'<<PROGRAM_NAME<<"\tsingle-exon\t"
		 ...
		 << min(nextPos+3,seqLen)<<"\t"<<exonScore<<"\t-\t0\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos+3,min(nextPos+3,seqLen),2,
			       REVERSE_STRAND);
	      break;
	    }
	  break;
	case NEG_GT: // internal-exon or initial-exon
	  switch(nextType)
	    ...
	    case NEG_ATG:
		 ...
		 << min(nextPos+3,seqLen)<<"\t"<<exonScore<<"\t-\t0\t"
		 << "transgrp=" << transcriptId << ";" << endl;
	      removeStopCodons(seqStr,thisPos+2,min(nextPos+3,seqLen),phase,
			       REVERSE_STRAND);
	      break;
	    }
	  break;
  ...
  os<<"# sequence length: " << seqLen << endl;
}



void GeneZilla::sample(int minLength,double gcContent,
		       BOOM::String isochoreFilename,
		       int contigId,BOOM::String outGffFile,
		       BOOM::String outFastaFile)
{
  // Preprocessing
  substrateId=contigId;
  alphabet=DnaAlphabet::global();
  seqStr=new String("AAAAAAAAAAA");
  seq=new Sequence(*seqStr,alphabet);
  processIsochoreFile(isochoreFilename,gcContent);
  Isochore *isochore=isochores.getIsochore(gcContent);
  Transitions *transitions=isochore->transitionProbs; // signal to signal
  FastaWriter fastaWriter;

  // Start in the intergenic state
  SignalType currentState=POLYA;
  SignalPtr signal=
    new Signal(-1,0,*isochore->signalTypeToSensor[POLYA],garbageCollector);
  BOOM::Stack<SignalPtr> revPath;
  revPath.push(signal);

  // Continue until enough DNA has been generated
  Sequence seq;
  int phase=0;
  ContentType contentType=INTERGENIC;
  while(seq.getLength()<minLength || contentType!=INTERGENIC)
    {
      // Choose the next signal type, which will determine the content type
      SignalType nextState=transitions->sample(currentState);
      SignalSensor *sigSensor=isochore->signalTypeToSensor[nextState];
      contentType=contentBetween(currentState,nextState);

      // Generate content state length
      DiscreteDistribution *distribution=
	isochore->contentToDistribution[contentType];
      int contentLen=distribution->sample();
      //cout<<contentType<<":"<<contentLen<<endl;

      // Update the phase
      int newPhase=
	updatePhase(phase,contentType,contentLen,nextState,*sigSensor);

      // Ensure that CDS lengths are divisible by 3
      adjustContentLen(contentLen,nextState,newPhase);

      // Generate content state emission
      ContentSensor *cSensor=isochore->contentToSensor[contentType];
      cSensor->sample(seq,contentLen,phase);

      // Generate next signal emission
      int signalPos=seq.getLength();
      sigSensor->sample(seq,true);
      phase=newPhase;

      // Change state
      currentState=nextState;
      signal=new Signal(signalPos,0,*sigSensor,garbageCollector);
      int signalPhase=getSignalPhase(*sigSensor,newPhase);
      signal->setPredecessor(signalPhase,revPath.top());
      revPath.push(signal);
    }

  // Reverse the path
  BOOM::Stack<SignalPtr> path;
  int pathLen=revPath.size();
  for(int i=0 ; i<pathLen ; ++i)
    path.push(revPath.pop());

  // Generate output
  BOOM::String defline=BOOM::String(">")+contigId+
    " random genome generated on "+BOOM::getDateAndTime();
  BOOM::String *seqStr=seq.toString(alphabet);
  generateGff(&path,seq.getLength(),0,outGffFile,*seqStr);
  fastaWriter.writeFasta(defline,*seqStr,outFastaFile);
  delete seqStr;
}



int GeneZilla::updatePhase(int phase,ContentType contentType,
			   int contentLen,int nextState,
			   SignalSensor &sensor)
{
  // First, propagate phase across the content region
  Strand strand=getStrand(contentType);
  bool isCoding=::isCoding(contentType);
  if(isCoding)
    if(strand==FORWARD_STRAND) phase+=contentLen;
    else phase-=contentLen;

  // Now propagate phase across the newest signal
  int winLen=sensor.getContextWindowLength();
  int consOff=sensor.getConsensusOffset();
  int consLen=sensor.getConsensusLength();
  int leftMargin=consOff;
  int rightMargin=winLen-consOff-consLen;
  switch(nextState)
    {
    case ATG: phase=rightMargin; break;
    case TAG: phase+=leftMargin; break;
    case GT: phase+=leftMargin; break;
    case AG: phase+=rightMargin; break;
    case PROM: phase=0; break;
    case POLYA: phase=0 ; break;
    case NEG_ATG: phase-=leftMargin; break;
    case NEG_TAG: phase=2-rightMargin; break;
    case NEG_GT: phase-=rightMargin; break;
    case NEG_AG: phase-=leftMargin; break;
    case NEG_PROM: phase=0; break;
    case NEG_POLYA: phase=0; break;
    }
...
void GeneZilla::adjustContentLen(int &contentLen,SignalType nextState,
				 int &newPhase)
{
  int delta;
  switch(nextState)
...
int GeneZilla::getSignalPhase(SignalSensor &sensor,int newPhase)
{
  int winLen=sensor.getContextWindowLength();
  int consOff=sensor.getConsensusOffset();
  int consLen=sensor.getConsensusLength();
  int leftMargin=consOff;
  int rightMargin=winLen-consOff-consLen;
  switch(sensor.getSignalType())
    {
    case ATG: return 0;
    case TAG: return 0;
    case GT: return newPhase;
    case AG: return posmod(newPhase-rightMargin);
    case PROM: return 0;
    case POLYA: return 0;
    case NEG_ATG: return 2;
    case NEG_TAG: return 2;
    case NEG_GT: return posmod(newPhase+rightMargin);
    case NEG_AG: return newPhase;
    case NEG_PROM: return 2;
    case NEG_POLYA: return 2;
    }
  throw "GeneZilla::getSignalPhase()";
}



void GeneZilla::removeStopCodons(BOOM::String &seqStr,int begin,int end,
				 int phase,Strand strand)
{
  Isochore *isochore=isochores.getIthIsochore(0);
  SignalSensor *sensor=strand==FORWARD_STRAND ?
    isochore->stopCodonSensor : isochore->negStopCodonSensor;
  if(strand==FORWARD_STRAND) begin+=(3-phase)%3;
  else begin+=(phase+1)%3;
  for(int pos=begin ; pos<end ; pos+=3)
    if(sensor->consensusOccursAt(seqStr,pos))
      {
...
void GeneZilla::removeStraddlingStops(BOOM::String &seqStr,int phase,
				      int thisPos,int nextPos,
				      Strand strand)
{
  BOOM::String codon="---";
  int tPos;
  if(strand==FORWARD_STRAND)
    {
      if(phase==1)
	{
	  codon[0]=seqStr[thisPos-1];
	  codon[1]=seqStr[nextPos+2];
	  codon[2]=seqStr[nextPos+3];
	  tPos=thisPos-1;
	}
      else if(phase==2)
	{
	  codon[0]=seqStr[thisPos-2];
	  codon[1]=seqStr[thisPos-1];
	  codon[2]=seqStr[nextPos+2];
	  tPos=thisPos-2;
	}
      if(codon=="TAG" || codon=="TGA" || codon=="TAA")
	{
	  int r=RandomNumber(3);
	  seqStr[tPos]=(r==0 ? 'A' : (r==1 ? 'C' : 'G'));
	}
    }
...
void GeneZilla::sampleTFBSs(int minLength,ContentSensor *intergenicSensor,
			    SignalSensor *TFBSsensor,int contigId,
			    BOOM::String outGffFile,
			    BOOM::String outFastaFile,
			    int meanIntergenicLen)
{
  ...
  seq=new Sequence(*seqStr,alphabet);
  FastaWriter fastaWriter;
  GeometricDistribution lenDist(meanIntergenicLen);
  ofstream os(outGffFile.c_str());
  SignalSensor *negTFBSsensor=TFBSsensor->reverseComplement();

  // Generate
  Sequence seq;
  int len=lenDist.sample();
  intergenicSensor->sample(seq,len,0);
  while(seq.getLength()<minLength)
    {
      int begin=seq.getLength()+1;
      SignalSensor *sigSensor=TFBSsensor;
      Strand strand=FORWARD_STRAND;
      if(Random0to1()<0.5)
	{
	  sigSensor=negTFBSsensor;
	  strand=REVERSE_STRAND;
	}
      sigSensor->sample(seq,false);
      int end=seq.getLength();
      os<<contigId<<"\trandom\tTFBS\t"<<begin<<"\t"<<end<<"\t"
	<<".\t"<<strand<<"\t."<<endl;
      len=lenDist.sample();
      intergenicSensor->sample(seq,len,0);
    }
