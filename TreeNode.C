...
void TreeNode::sample(Sequence &appendToThis)
{
  if(hasPartition())
    {
      double P=exp(left->branchLogP);
      if(Random0to1()<P)
	left->sample(appendToThis);
      else
	right->sample(appendToThis);
    }
  else submodel->sample(appendToThis);
}
