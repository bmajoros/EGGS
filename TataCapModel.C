...
void TataCapModel::sample(Sequence &seq)
{
    tataModel->sample(seq);
    int range=maxSeparation-minSeparation+1;
    int separation=minSeparation+RandomNumber(range);
    intergenicModel->sample(seq,separation,0);
    capModel->sample(seq);
}
