...
void MarkovChain::sample(Sequence &seq,int length,int)
{
  BOOM::String ngram;
  int L=seq.getLength();
  for(int i=0 ; i<length ; ++i)
	...
	{
	  BOOM::StringMap<double> &model=*models[order];
	  seq.toString(alphabet,L-order,order,ngram);
	  ngram+='A';
	  if(model.isDefined(ngram.c_str(),order+1)) break;
	  --order;
	}
      ngram.chop();
      double P=Random0to1();
      for(Symbol s=0 ; s<5 ; ++s)
	{
	  ...
	  if(c=='N') continue;
	  BOOM::String ngramPlus=ngram+c;
	  double logP=models[order]->lookup(ngramPlus.c_str(),order+1);
	  double sector=exp(logP);
	  P-=sector;
	  if(P<=0 || c=='T')
	    {
	      seq.append(s);
