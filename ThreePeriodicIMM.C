...
ContentSensor *ThreePeriodicIMM::reverseComplement()
{
  if(!revComp)
    {
      revComp=
	new ThreePeriodicIMM(complement(getStrand()),
...
void ThreePeriodicIMM::sample(Sequence &appendToThis,int length,
			      int phase)
{
  Strand strand=getStrand();
  int update=(strand==FORWARD_STRAND ? 1 : 4);
  for(int i=0 ; i<length ; ++i)
    {
      chains[phase]->sample(appendToThis,1,0);
      phase=(phase+update)%3;
    }
}
