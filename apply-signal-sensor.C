...
int Application::main(int argc,char *argv[])
  {
    ...
    CommandLine cmd(argc,argv,"c:n:b:");
		       ...
		       String label)
{
  ...
  for(int i=0 ; i<=seqLen-sigLen ; ++i) { // ### <= was <
    ...
    //if(ptr) {
    //double score=ptr->contextWindowScore();
    //cout<<"score="<<score<<" cutoff="<<sensor->getCutoff()<<" fgScore="<<fgScore<<endl;//<<" bgScore="<<background->scoreSubsequence(S,seq,i,sigLen,0)<<endl;
    if(score>=sensor->getCutoff()) {
      if(desiredCount>=0) scores.push_back(score);
      else cout<<ID<<"\tSignalFinder\t"<<label<<"\t"<<i<<"\t"
	       <<(i+sigLen)<<"\t"<<score<<"\t+\t."<<endl;
    }
    if(revScore>=revSensor->getCutoff()) {
      if(desiredCount>=0) scores.push_back(revScore);
      else cout<<ID<<"\tSignalFinder\t"<<label<<"\t"<<i<<"\t"
	       <<(i+sigLen)<<"\t"<<revScore<<"\t-\t."<<endl;
    }
  }
}
